...
global $post, $post_id, $slice, $months_opts;
if ( ! $post_id )
	$post_id = $post->ID;
...
<section id="c-<?= $course['meta_id'] ?>" class="foldout">
 <header><h2><?php
	...
	if ( in_array( $_cat[0]->slug, array( 'sprachkurs', '50plus' ) ) ) {
		$_lang = wp_get_object_terms( $post_id, 'lang' );
		echo wptexturize( $_lang[0]->name . 'kurs "' . trim($course['name']) . '"' );
	} else {
		echo wptexturize( $course['name'] );
	}
 ?></h2><?= $course_tags ?></header>
...
<?php if ( ! empty( $course['hpw'] ) ) : ?>
   <tr>
    ...
    <td><?= $course['hpw'] ?> Lektionen<?php
   	if ( ! empty($course['mpl']) ) : ?> à <?= $course['mpl'] ?> Minuten<?php endif;
   	?> pro Woche
<?php endif;
...
if ( count($course['cost']) > 1 && 's' == $course['cost']['period'] ) :
   ...
   <tr>
    <th>Kursdauer
    <td><?php
    		echo empty($course['dur'][1]) ? 1 : (int)($course['dur'][1] / $course['dur'][0]), ' Semester à ' . $course['dur'][0] . ' Wochen';
...
<?php
	...
	if ( empty( $_tuitions['add'] ) ) {
		...
		foreach ( fbk_tt_get_intervals_by_timeline( $_intervals['base'] ) as $ibase )
			$intervals[ $ibase[2] ] = array( array_slice( $ibase, 0, 2 ) ); // For consistency with multi-interval data as seen below
	} else {
...
<?php
			...
			foreach ( $row as $week => $price ) :
				...
				$j++;
?>
    <th<?php /* data-dur="<?= $week ?>" */ ?>><?php if ( $is_semester )
    		echo "$week Semester";
    	else
